...
def sign(signing_key, signing_private_key):
...
@app.route('/subscribe/on_subscribe', methods=['POST'])
def onsubscribe():
    data = request.get_json()
    print(f"/subscribe/on_subscribe called :: Request -> {data}")
...
def serve_file():
    os.system(f'python -m http.server {static_file_port} --directory ondc-verification')
