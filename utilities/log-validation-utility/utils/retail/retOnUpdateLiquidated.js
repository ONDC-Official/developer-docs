...
const checkOnUpdateLiquidated = (dirPath, msgIdSet, state) => {
  try {
    let on_update = fs.readFileSync(
      dirPath + `/${constants.RET_ONUPDATE}_liquidated.json`
    );
    ...
    let liquidatedObj = {};
    liquidatedObj = checkOnUpdate(msgIdSet, on_update, state);
    ...
    try {
      ...
      if (_.gte(dao.getValue("tmstmp"), contextTime)) {
        liquidatedObj.tmpstmp1 = `Timestamp for ${compareApi} api cannot be greater than or equal to /${constants.RET_ONUPDATE}_${state} api`;
      }
...
module.exports = checkOnUpdateLiquidated;
