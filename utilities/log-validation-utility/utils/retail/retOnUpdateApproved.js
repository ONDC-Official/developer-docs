...
const checkOnUpdateApproved = (dirPath, msgIdSet, state) => {
  try {
    let on_update = fs.readFileSync(
      dirPath + `/${constants.RET_ONUPDATE}_approved.json`
    );
    ...
    let approvedObj = {};
    approvedObj = checkOnUpdate(msgIdSet, on_update, state);
    ...
    try {
      ...
      if (_.gte(dao.getValue("tmstmp"), contextTime)) {
        approvedObj.tmpstmp1 = `Timestamp for ${compareApi} api cannot be greater than or equal to /${constants.RET_ONUPDATE}_${state} api`;
      }
...
module.exports = checkOnUpdateApproved;
